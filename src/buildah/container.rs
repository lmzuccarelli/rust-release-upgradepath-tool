...
use crate::buildah::buildah_error::BuildahError;
use crate::buildah::image::Image;
use crate::buildah::mount::Mount;
use crate::buildah::parsing::*;
...
impl Container {
    ...
    #[tracing::instrument]
    pub fn copy<P1: AsRef<Path> + Debug, P2: AsRef<Path> + Debug>(
        &mut self,
        src: P1,
        dest: P2,
    ) -> Result<(), BuildahError> {
    ...
    pub fn untar(&self, file: &str, dest: &str) -> Result<(), BuildahError> {
        let tar_gz = File::open(file).expect("could not open file");
        let tar = GzDecoder::new(tar_gz);
        let mut archive = Archive::new(tar);
        let res = archive.unpack(dest);
        if res.is_err() {
            error!("Could not unpack tarball: {:?}", res);
        }
        Ok(())
    }
...
impl Drop for Container {
    #[tracing::instrument]
    fn drop(&mut self) {
        let output = Command::new("buildah").arg("rm").arg(&self.name).output();
